...
assert_77() {
  ...
  check "$logfile" "durationMs=.*";                         [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "replay-save-header"
}
...
# TEST 80: replay overflow — record past REPLAY_MAX_EVENTS, verify truncation + counter
assert_80() {
  local ok=0 logfile="$1"
  check "$logfile" "NeonWave: REPLAY overflow recorded=32773 stored=32768"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
...
# Bot type tests (v0.80)
assert_96() {
  local ok=0 logfile="$1"
  check "$logfile" "HEALER"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "bot-healer"
}
assert_97() {
  local ok=0 logfile="$1"
  check "$logfile" "SHIELD"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "bot-shielder"
}
assert_98() {
  local ok=0 logfile="$1"
  check "$logfile" "ELITE"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "bot-elite"
}

# Test 99: Seasonal rotation marker
assert_99() {
  local ok=0 logfile="$1"
  check "$logfile" "SEASONAL rotation"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "seasonal-rotation"
}

# Test 100: Arena validation — arena scaling summary present
assert_100() {
  local ok=0 logfile="$1"
  check "$logfile" "arena drone scaling"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "arena-validation"
}

# Test 101: Performance stress — high drone count
assert_101() {
  local ok=0 logfile="$1"
  check "$logfile" "arena drone count scale 2.00"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "performance-stress"
}

# Test 102: Controller active — joystick enabled, ghost kit on
assert_102() {
  local ok=0 logfile="$1"
  check "$logfile" "NeonWave: GHOST kit active"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "controller-active"
}

# Test 103: Chronomancer boss
assert_103() {
  local ok=0 logfile="$1"
  check "$logfile" "CHRONOMANCER"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "boss-chronomancer"
}

# Test 104: Void Walker boss
assert_104() {
  local ok=0 logfile="$1"
  check "$logfile" "VOID WALKER"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
...
# Test 111: Ghost Loadout Balance - Infiltrator (default, 80 energy)
assert_111() {
  local ok=0 logfile="$1"
...
# Test 112: Ghost Loadout Balance - Saboteur (70 energy, cheaper EMP/Lockdown)
assert_112() {
  local ok=0 logfile="$1"
  check "$logfile" "Ghost: .* ENERGY=70 (loadout 1)"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "ghost-balance-saboteur"
}

# Test 113: Ghost Loadout Balance - Spectre (90 energy, no cloak, has nuke)
assert_113() {
  local ok=0 logfile="$1"
...
# Test 114: Ghost Loadout Balance - Saboteur EMP Discount
# Saboteur EMP costs 25 (35-10 discount), Lockdown costs 35 (50-15 discount)
# With 70 energy: can cast 2 EMPs (50) or 1 EMP + 1 Lockdown (60)
assert_114() {
  local ok=0 logfile="$1"
  check "$logfile" "Ghost: .* ENERGY=70 (loadout 1)"; [ $LAST_RESULT -eq 0 ] || ok=1
  check "$logfile" "Ghost: .* joined the Ghost team (loadout 1)"; [ $LAST_RESULT -eq 0 ] || ok=1
  no_fatal_warnings "$logfile" || ok=1
  report $ok "ghost-balance-saboteur-discount"
}

# Test 115: loadout next (g_ghost_cycletest) 0 → 1 Saboteur
assert_115() {
  local ok=0 logfile="$1"
...
# Test 118: Frostbite/Ironman share oa_minia but not look or loop
assert_118() {
  local ok=0 logfile="$1"
  local hdr="$TESTDIR/../oa-gamecode/code/game/neon_maplook.h"
  local ui="$TESTDIR/../oa-gamecode/code/q3_ui/ui_neonstart.c"
  ...
  grep -q 'NW_GRID_IRON' "$hdr" || ok=1
  grep -q 'NW_MapLookArena' "$hdr" || ok=1
  grep -q '"FROST. Ice look. Slow drones."' "$ui" || ok=1
  grep -q ', 12,' "$ui" || ok=1
  grep -q 'NeonStart_Draw' "$ui" || ok=1
  grep -q 'PLAY  INFILTRATOR' "$ui" || ok=1
  grep -q 'TODAY' "$ui" || ok=1
  grep -q 'NeonArena_Draw' "$ui" || ok=1
  grep -q '"150"' "$ui" || ok=1
  grep -q 'g_neonwave_modifier": 12' "$TESTDIR/../configs/arenas/frostbite.json" || ok=1
